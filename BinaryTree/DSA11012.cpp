...
bool CheckNode(Node* root, int x, int y, char side){
    if(root == NULL)    return false;
    if(root->value == x){
        if(root->left->value == y && side == 'L')
            return true;
        if(root->right->value == y && side == 'R')
            return true;
    }
    else
        return (CheckNode(root->left, x, y, side) || CheckNode(root->right, x, y, side));
    return false;
}
/*Initialize*/
int test, n, m;
...
void solve(){
    ...
    Fu(i, 0, n, 1){
        cin >> x >> y >> side;
        if(res && !CheckNode(root, x, y, side))
            res = 0;
    }
    cout << res;
}
